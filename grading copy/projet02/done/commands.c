...
int program_shrink(program_t* program){
    ...
    // correcteur: still non-compliant with the specified behaviour (see the handout)
...
int program_add_command(program_t* program, const command_t* command){
    ...
    M_REQUIRE_NON_NULL(program);
    M_REQUIRE_NON_NULL(program->listing);
    // correcteur: @see https://moodle.epfl.ch/mod/forum/discuss.php?d=17763#p35318
    // correcteur: still missing check of invalid order (hint: authorized enum value)
    // correcteur: still missing check of invalid type
    // correcteur: still missing check of invalid write (hint: write data too large for write size)
    if(command->type == INSTRUCTION) {
    ...
    while (program->nb_lines * sizeof(command_t) >= program->allocated) {
        // correcteur: still missing overflow check
...
int program_read(const char* filename, program_t* program){
    ...
    // correcteur: still missing handling of error code of program_init
...
int handle_read(FILE* input, command_t* command){
    ...
    // correcteur: still missing initialization of write_data to a default value (hint: 0)
	return error;
}
...
int handle_write(FILE* input, command_t* command){
    ...
    if(type == 'I') {
        command->type = INSTRUCTION;
        // correcteur: missing update of error code
...
int handle_read_data(FILE* input, command_t* command) {
    ...
    // correcteur: potential override of error with ERR_NONE
    error = set_vaddr(input, command);
    return error;
}
